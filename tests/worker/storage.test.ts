...
function result(checkedAt: number, overrides: Partial<CheckResult> = {}): CheckResult {
  return {
    monitorId: "blog",
    checkedAt,
    success: true,
...
function failedResult(checkedAt: number, overrides: Partial<CheckResult> = {}): CheckResult {
...
function persisted(previous: MonitorState | null, checkResult: CheckResult): PersistedCheck {
  return {
    result: checkResult,
...
describe("scheduled-check persistence", () => {
  it("keeps D1 and the public API aligned across the complete default threshold timeline", async () => {
    ...
    async function apply(
      ...
      if (transition.notification) notifications.push(transition.notification.type);
    ...
    expect(await rows("SELECT id, highest_severity, recovered_at FROM incidents")).toEqual([
      {
    ...
    expect(
      await rows("SELECT check_count, failed_check_count, highest_severity FROM daily_summaries"),
    ).toEqual([{ check_count: 6, failed_check_count: 3, highest_severity: "outage" }]);
  });

  it("stores one successful initial state and daily aggregate without a raw row", async () => {
    const checkResult = result(Date.UTC(2026, 7, 5, 12));

    await expect(persistCheckBatch(env.DB, [persisted(null, checkResult)])).resolves.toEqual({
      appliedMonitorIds: ["blog"],
    });

    expect(await rows("SELECT name FROM sqlite_master WHERE type = 'table'")).not.toContainEqual({
      name: "check_results",
    });
  ...
  it("aggregates null and non-null response times independently", async () => {
    const first = persisted(null, result(Date.UTC(2026, 7, 5, 12), { responseMs: null }));
  ...
  it("keeps daily summaries for multiple PoPs separate and maps an empty location to unknown", async () => {
    ...
    const unknown = persisted(null, result(at + 1, { location: "", responseMs: 50 }));
  ...
  it("opens, escalates, and recovers the same incident while preserving outage severity", async () => {
    ...
    const second = persisted(first.transition.next, failedResult(Date.UTC(2026, 7, 5, 12, 1)));
    const third = persisted(second.transition.next, result(Date.UTC(2026, 7, 5, 12, 2)));
    ...
    expect(
      await rows("SELECT id, outage_at, recovered_at, highest_severity FROM incidents"),
    ).toEqual([
      {
        id: "blog:1785931200000",
  ...
  it("never downgrades a daily maximum from outage to degraded or operational", async () => {
    const first = persisted(null, failedResult(Date.UTC(2026, 7, 5, 12)));
    const outage = persisted(first.transition.next, failedResult(Date.UTC(2026, 7, 5, 12, 1)));
    const recovered = persisted(outage.transition.next, result(Date.UTC(2026, 7, 5, 12, 2)));
    ...
    expect(
      await rows("SELECT check_count, failed_check_count, highest_severity FROM daily_summaries"),
    ).toEqual([{ check_count: 4, failed_check_count: 3, highest_severity: "outage" }]);
  });

  it("applies fresh monitors while skipping a duplicate monitor in the same batch", async () => {
    const at = Date.UTC(2026, 7, 5, 12);
    ...
    await expect(persistCheckBatch(env.DB, [fresh, duplicate])).resolves.toEqual({
      appliedMonitorIds: ["vault"],
    });
    expect(
      await rows("SELECT monitor_id, check_count FROM daily_summaries ORDER BY monitor_id"),
    ).toEqual([
      { monitor_id: "blog", check_count: 1 },
      { monitor_id: "vault", check_count: 1 },
    ]);
    expect(await rows("SELECT monitor_id FROM monitor_state ORDER BY monitor_id")).toEqual([
      { monitor_id: "blog" },
      { monitor_id: "vault" },
    ]);
    expect(await rows("SELECT monitor_id FROM incidents")).toEqual([{ monitor_id: "vault" }]);
  });

  it("preserves D1 validation errors unchanged", async () => {
    const invalid = result(Date.UTC(2026, 7, 5, 12), { responseMs: -1 });

    await expect(persistCheckBatch(env.DB, [persisted(null, invalid)])).rejects.toThrow();
  });

  it("ignores a transition already known to be stale", async () => {
    ...
    await expect(
      persistCheckBatch(env.DB, [{ result: olderResult, transition: staleTransition }]),
    ).resolves.toEqual({ appliedMonitorIds: [] });

    expect(await rows("SELECT check_count, failed_check_count FROM daily_summaries")).toEqual([
      { check_count: 1, failed_check_count: 0 },
    ]);
    expect((await loadMonitorStates(env.DB, ["blog"])).get("blog")?.latest.checkedAt).toBe(
      1_785_931_320_000,
    );
    expect(await rows("SELECT COUNT(*) AS count FROM incidents")).toEqual([{ count: 0 }]);
  });

  it("rejects an older state, aggregate, and incident mutation inside the D1 batch", async () => {
    const newer = persisted(null, result(Date.UTC(2026, 7, 5, 12, 2)));
    await persistCheckBatch(env.DB, [newer]);
    ...
    await expect(persistCheckBatch(env.DB, [olderIncidentTransition])).resolves.toEqual({
      appliedMonitorIds: [],
    });

    expect(
      await rows("SELECT check_count, failed_check_count, highest_severity FROM daily_summaries"),
    ).toEqual([{ check_count: 1, failed_check_count: 0, highest_severity: "operational" }]);
    expect((await loadMonitorStates(env.DB, ["blog"])).get("blog")).toEqual(newer.transition.next);
    expect(await rows("SELECT COUNT(*) AS count FROM incidents")).toEqual([{ count: 0 }]);
  });

  it("returns immediately for empty state IDs and empty write batches", async () => {
    await expect(persistCheckBatch(env.DB, [])).resolves.toEqual({ appliedMonitorIds: [] });
...
describe("bounded reads", () => {
  it("bounds state and summary reads by monitor IDs and inclusive day", async () => {
    const blog = persisted(null, result(Date.UTC(2026, 7, 5), { monitorId: "blog" }));
    const vaultOld = persisted(null, result(Date.UTC(2026, 7, 4), { monitorId: "vault" }));
    ...
    expect(await readCurrentStates(env.DB, ["blog"])).toEqual([blog.transition.next]);
    expect(await readCurrentStates(env.DB, [])).toEqual([]);
    expect(await readDailySummariesSince(env.DB, ["vault"], "2026-08-05")).toEqual([
      {
        monitorId: "vault",
        day: "2026-08-05",
        location: "sfo",
