...
describe('HistoryStrip', () => {
  it('renders every one of 90 days as a focusable button with complete non-color labels', () => {
    ...
    expect(
      buttons.every(
        (button) => button.firstElementChild?.getAttribute('aria-hidden') === 'true',
      ),
    ).toBe(true)
  ...
  it('selects the same single detail popover with hover, focus, and click', async () => {
    ...
    expect(
      screen.getAllByRole('region', { name: /history details/i }),
    ).toHaveLength(1)
    expect(
      screen.getByRole('region', { name: /history details/i }),
  ...
  it('closes on Escape or when focus leaves the strip', async () => {
    ...
    expect(
      screen.queryByRole('region', { name: /history details/i }),
