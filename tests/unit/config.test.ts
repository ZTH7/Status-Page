...
function sources(
  ...
  return {
    ...
    wranglerConfig: {
      triggers: { crons: overrides.crons ?? ['* * * * *', '17 3 * * *'] },
    },
...
describe('parseConfigSources', () => {
  ...
  it.each([
    ...
    [
      'invalid monitor ID',
      sources({ monitors: [{ ...monitor, id: 'Invalid_ID' }] }),
    ],
    [
    ...
    [
      'non-HTTP site URL',
      sources({ site: { url: 'mailto:ops@example.test' } }),
    ],
    [
      'site timeout below one second',
      sources({ site: { requestTimeoutSeconds: 0 } }),
    ],
    [
    ...
    [
      ...
      sources({ monitors: [{ ...monitor, expectStatus: 99 }] }),
    ],
    [
      'status code above 599',
      sources({ monitors: [{ ...monitor, expectStatus: 600 }] }),
    ],
    [
      'zero site threshold',
      sources({ site: { thresholds: { degradedAfterFailures: 0 } } }),
    ],
    [
    ...
    [
      'unsupported method',
      sources({ monitors: [{ ...monitor, method: 'POST' }] }),
    ],
    ...
    ['unsupported cadence', sourcesWithCrons(['*/2 * * * *', '17 3 * * *'])],
    ['missing retention Cron', sourcesWithCrons(['*/5 * * * *'])],
    [
    ...
    [
      'raw retention above 365 days',
      sources({ site: { rawRetentionDays: 366 } }),
    ],
    [
      'raw retention shorter than history',
      sources({ site: { historyDays: 91, rawRetentionDays: 90 } }),
    ],
    [
...
describe('config generator', () => {
  it('emits deterministic public artifacts without monitor targets', () => {
    ...
    expect(firstRun[1]).not.toMatch(
      /https:\/\/(?:www|vault|tools|drive)\.zdaily\.net/,
    )
    expect(firstRun[2]).not.toMatch(
      /https:\/\/(?:www|vault|tools|drive)\.zdaily\.net/,
    )
  ...
  it('rejects a public asset symlink that resolves outside public', () => {
    ...
    try {
      ...
      externalDirectory = mkdtempSync(
        join(tmpdir(), 'cfstatuspage-config-assets-'),
      )
