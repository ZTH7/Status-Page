...
function compositeKey(monitorId: string, day: string, location: string): string {
      ...
      const result = batch.map((query) => {
        if (/^\s*INSERT INTO daily_summaries/i.test(query.sql)) {
          const [
            ...
            responseCount,
            highestSeverity,
          ] = query.params as [string, string, string, number, number, number, number, string];
        ...
        if (/^\s*SELECT/i.test(query.sql)) {
          const [monitorId, day, location] = query.params as [string, string, string];
          ...
          return {
            ...
            results:
              verificationRow === undefined
                ? []
                : Array.from({ length: options.verificationRowCopies ?? 1 }, () => verificationRow),
            meta: { changes: 0 },
          };
        }
...
function monitor(input: Record<string, unknown>, id = "blog"): Record<string, unknown> {
...
function check(
  ...
  return (monitor(input, id).checks as Record<string, Record<string, unknown>>)[day]!;
}
...
function location(
  ...
  return (check(input, day, id).res as Record<string, Record<string, unknown>>)[pop]!;
}
...
describe("parseLegacyState", () => {
  ...
  it.each([
    [
      "invalid firstCheck",
      (input: Record<string, unknown>) => (monitor(input).firstCheck = "2026-02-30"),
    ],
    [
      "invalid check date",
      (input: Record<string, unknown>) => {
    ...
    ["non-object lastCheck", (input: Record<string, unknown>) => (monitor(input).lastCheck = null)],
    [
    ...
    ["non-object res", (input: Record<string, unknown>) => (check(input).res = [])],
  ...
  it.each([
    ["negative fails", (input: Record<string, unknown>) => (check(input).fails = -1)],
    ["fractional fails", (input: Record<string, unknown>) => (check(input).fails = 0.5)],
    [
      "non-finite fails",
      (input: Record<string, unknown>) => (check(input).fails = Number.POSITIVE_INFINITY),
    ],
    [
      "unsafe fails",
      (input: Record<string, unknown>) => (check(input).fails = Number.MAX_SAFE_INTEGER + 1),
    ],
    ["negative n", (input: Record<string, unknown>) => (location(input).n = -1)],
    ["fractional n", (input: Record<string, unknown>) => (location(input).n = 1.5)],
    ["non-finite n", (input: Record<string, unknown>) => (location(input).n = Number.NaN)],
    [
      "unsafe n",
      (input: Record<string, unknown>) => (location(input).n = Number.MAX_SAFE_INTEGER + 1),
    ],
    ["negative ms", (input: Record<string, unknown>) => (location(input).ms = -1)],
    [
      "non-finite ms",
      (input: Record<string, unknown>) => (location(input).ms = Number.POSITIVE_INFINITY),
    ],
    ["unsafe ms", (input: Record<string, unknown>) => (location(input).ms = Number.MAX_VALUE)],
    ["negative average", (input: Record<string, unknown>) => (location(input).a = -1)],
    ["non-finite average", (input: Record<string, unknown>) => (location(input).a = Number.NaN)],
    ["unsafe average", (input: Record<string, unknown>) => (location(input).a = Number.MAX_VALUE)],
  ])("rejects %s", (_name, mutate) => {
    const input = validInput();
    mutate(input);
    expect(() => parseLegacyState(input)).toThrow(/finite|non-negative|integer/i);
  });
  ...
  it("rejects prototype-bearing and prototype-keyed records", () => {
    const inheritedRoot = Object.create({ monitors: {} }) as Record<string, unknown>;
    expect(() => parseLegacyState(inheritedRoot)).toThrow(/object shape|prototype/i);
...
describe("convertLegacyState", () => {
  it("preserves aggregates and fills successful days omitted by disabled response-time collection", () => {
    const converted = convertLegacyState(parseLegacyState(JSON.parse(fixtureText)));

    expect(converted.filter((row) => row.checkCount > 0)).toEqual([
      {
        monitorId: "blog",
        day: "2026-08-01",
        location: "HKG",
    ...
    expect(converted.filter((row) => row.checkCount === 0)).toEqual(
      [
        ["blog", "2026-08-04"],
        ["retired-api", "2026-08-02"],
        ["retired-api", "2026-08-03"],
        ["retired-api", "2026-08-04"],
        ["vault", "2026-08-03"],
        ["vault", "2026-08-04"],
      ].map(([monitorId, day]) => ({
        monitorId,
  ...
  it("uses each monitor's latest explicit day when the global last update is absent", () => {
    const input = JSON.parse(fixtureText) as Record<string, unknown>;
    delete input.lastUpdate;

    const converted = convertLegacyState(parseLegacyState(input));

    expect(converted.some((row) => row.monitorId === "blog" && row.day === "2026-08-04")).toBe(
      false,
    );
    expect(converted.some((row) => row.monitorId === "vault" && row.day === "2026-08-03")).toBe(
      false,
    );
    expect(
      converted.filter((row) => row.monitorId === "retired-api" && row.checkCount === 0),
    ).toHaveLength(3);
  });
});
...
describe("buildMigrationStatements", () => {
  it("builds deterministic bound replacement UPSERTs that only target daily_summaries", () => {
    ...
    expect(first[0]?.params).toEqual(["blog", "2026-08-01", "HKG", 1, 0, 130, 1, "operational"]);
    expect(first.every((query) => query.sql.includes("INSERT INTO daily_summaries"))).toBe(true);
...
describe("runMigration", () => {
  ...
  it.each([
    [
      ...
      rawLegacyJson(`"blog":${minimalMonitorJson},"\\u0062log":${minimalMonitorJson}`),
    ],
    [
      "checks date member",
      rawLegacyJson(
  ...
  ])("rejects a duplicate raw JSON %s before backup or D1 access", async (_name, rawJson) => {
  ...
  ])("rejects unsafe accumulated source %s before backup or D1 access", async (_name, monitors) => {
  ...
  it("requires all credentials before config, backup, or network work and lists every missing name", async () => {
    ...
    ).rejects.toThrow(/CF_ACCOUNT_ID.*CF_API_TOKEN.*KV_NAMESPACE_ID.*D1_DATABASE_ID/s);
  ...
  it("backs up before write, batches at most 100 bound UPSERTs, and verifies every key", async () => {
    ...
    expect(verificationBatches.map((batch) => batch.length)).toEqual([100, 100, 5]);
    expect(cloudflare.d1Batches.flat().every((query) => query.params.length > 0)).toBe(true);
    expect(
      cloudflare.requests
  ...
  it("repeated writes replace aggregates and keep the verified composite row count stable", async () => {
    ...
    expect(firstReport.verifiedRowCount).toBe(14);
    expect(secondReport.verifiedRowCount).toBe(14);
    expect(cloudflare.rows).toHaveLength(14);
    expect(cloudflare.rows.get(compositeKey("blog", "2026-08-01", "HKG"))).toMatchObject({
      response_time_sum: 999,
    });
  });

  it.each([
    ["check_count", (row: StoredSummaryRow) => ({ ...row, check_count: row.check_count + 1 })],
    [
      "response_time_sum",
    ...
    ["highest_severity", (row: StoredSummaryRow) => ({ ...row, highest_severity: "outage" })],
  ])("rejects a verification %s mismatch", async (_field, mutateVerificationRow) => {
  ...
  it.each([
    ["Cloudflare failure", { success: false, errors: [], messages: [], result: [] }],
    ["malformed result", { success: true, errors: [], messages: [], result: {} }],
  ...
  it.each([
    ...
    [
      "non-2xx response",
      async (_batch: D1Query[]) => new Response("SECRET_RESPONSE_BODY test-token", { status: 503 }),
    ],
    [
      "invalid JSON response",
      async (_batch: D1Query[]) => new Response("SECRET_RESPONSE_BODY test-token", { status: 200 }),
    ],
    [
    ...
    [
      ...
      async (batch: D1Query[]) =>
        Response.json({
          ...
          errors: [{ message: "SECRET_RESPONSE_BODY test-token" }],
          messages: [],
          result: batch.slice(1).map(() => ({ success: true, results: [], meta: {} })),
        }),
    ],
  ])(
